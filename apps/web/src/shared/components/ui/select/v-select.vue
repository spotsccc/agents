...
<template>
  <Select v-model="model" :disabled>
    ...
    <SelectContent v-if="!loading">
      <SelectItem v-for="currency in items" :key="currency.id" :value="currency.id">
        {{ currency.label }}
      </SelectItem>
      <SelectLabel v-if="items.length === 0"> Not found </SelectLabel>
    </SelectContent>
    <SelectContent v-else> loading </SelectContent>
    <p>{{ error }}</p>
  </Select>
</template>
