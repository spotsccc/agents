...
import type { CreateTransferTransactionRequest } from "./contract.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await db.transaction().execute(async (trx) => {
      ...
      const fromWalletCheck = await trx
        .selectFrom("wallets")
        .select("id")
      ...
      if (toWalletCheck.length === 0) {
        throw new Error(
          "Destination wallet not found or does not belong to user"
        );
      }

      // Get source wallet balance with lock
      const fromBalanceResult = await sql<{ balance: number }>`
        SELECT balance FROM wallet_balances
        WHERE wallet_id = ${fromWalletId} AND currency_code = ${fromCurrency}
        FOR UPDATE
      `.execute(trx);
      ...
      // Get destination wallet balance with lock
      const toBalanceResult = await sql<{ balance: number }>`
        SELECT balance FROM wallet_balances
        WHERE wallet_id = ${toWalletId} AND currency_code = ${toCurrency}
        FOR UPDATE
      `.execute(trx);
      ...
      if (toNewBalance > MAX_BALANCE) {
        throw new Error(
          `Transfer would exceed maximum balance for ${toCurrency}`
        );
      }
      ...
      // Create debit entry (source wallet - negative amount)
      ...
      // Create credit entry (destination wallet - positive amount)
